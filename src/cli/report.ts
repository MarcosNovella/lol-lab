...
/**
 * `lol report [cuenta] [partidas] [rol]` — the three most expensive moments of each recent game,
 ...
 * the week it is generated. That is a property of the game, not of the tool, and the report says
 * so rather than pretending otherwise.
 */

export function run(argv: string[]): void {
  const [accountArg = 'smurf', gamesArg = '5', roleArg = 'mid'] = argv;
  ...
  try {
    ...
    out(
      `Últimas ${rows.length} partidas de ${account.label ?? account.gameName} (${roleArg}, soloq)`,
    );
    out('');
...
export const SUMMARY = 'los 3 momentos más caros de cada partida reciente, con el minuto exacto';
export const USAGE = 'lol report [cuenta] [partidas] [rol]';
