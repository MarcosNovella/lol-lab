...
import { catalogVersions, hasCatalogForPatch, patchesInCache, saveItems } from './store/items.ts';
...
export async function bajarCatalogos(db: Db, onProgress?: UpkeepProgress): Promise<ItemsResult> {
  ...
  for (const { patch } of patches) {
    ...
    saveItems(db, await fetchItems(version));
    result.bajados += 1;
  }
  return result;
}
...
export async function bajarImagenes(db: Db, onProgress?: UpkeepProgress): Promise<AssetsResult> {
  ...
  onProgress?.(3, 3, 'listo');

  return {
    bajadas: champs.downloaded + items.downloaded + map.downloaded,
    yaEstaban: champs.skipped + items.skipped + map.skipped,
    sinArte: champs.missing.length + items.missing.length,
    megas: (champs.bytes + items.bytes + map.bytes) / 1024 / 1024,
    bloqueado: null,
  };
}
